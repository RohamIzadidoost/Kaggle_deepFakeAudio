...
fixes the recording domain. Its advertised feature-refinement step is also
structurally degenerate: non-negative matrix factorisation applied to a
two-dimensional class-posterior vector cannot carry the claimed parts-based
structure. We then build a benchmark on which the claim can be tested fairly.
...
\midrule
ASVspoof 2021 (DF track) & Telephone / codec-degraded deepfakes & 16{,}977 & 441{,}894 \\
LibriSpeech + TTS set    & LibriSpeech real speech + modern TTS fakes & 2{,}274 & 2{,}173 \\
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners=2pt, align=center, inner sep=4pt,
              font=\footnotesize, minimum height=8mm, text width=24mm},
  src/.style={box, text width=21mm},
  arr/.style={-{Latex[length=2mm]}, thick}
]
\node[src] (a) {ASVspoof 2021 DF\\\scriptsize 16{,}977 R / 441{,}894 F};
\node[src, below=of a] (b) {LibriSpeech+TTS\\\scriptsize 2{,}274 R / 2{,}173 F};
...
\node[box, right=7mm of w] (s) {speaker-disjoint\\\scriptsize StratifiedGroupKFold};
...
digital silence. Roughly half of the ASVspoof audio entered training as silence.

\paragraph{Why it is worse than random corruption}
The corruption is not distributed uniformly over the benchmark. ASVspoof is the
only source contributing a large quantity of \emph{real} speech, so the silent
clips fall predominantly in the real class. A model presented with this data has
...
\texttt{soundfile} first for speed, and on any exception fall back to librosa's
FFmpeg-backed loader~\cite{librosa}, which reads the affected files correctly.
Every number reported in this paper is post-fix.
...
Reported figures are single runs at a fixed seed (42). We state this plainly
rather than implying more; Sect.~\ref{sec:limits} discusses what it does and
does not license.
...
is better. The proposed model roughly halves the best classical configuration's
EER, and the audited paper's headline configuration (Gaussian Naive Bayes over
MFCCs) is the weakest of the nine.}
\label{tab:indomain}
...
Gaussian Naive Bayes \emph{(audited headline)} & MFCC     & 26.69 & 73.05 & 71.75 & 0.81 \\
\bottomrule
\end{tabular}
\end{table}
...
attention weights $\alpha_t$ of Eq.~\eqref{eq:attn}, at 160\,ms resolution.}
\label{fig:explain}
\end{figure}
...
The LibriSpeech+TTS fold is more informative than the others, because 0.42 AUC
...
LibriSpeech+TTS real speech is clean studio audiobook audio, so the learned rule
...
LibriSpeech+TTS set's $\sim$2.3k clips, so that row should be read as a lower
bound rather than a clean measurement. And MLAAD's fake-only composition means
...
unreadable files rather than zero-filling silently.
...
LibriSpeech+TTS set, and MLAAD here), different detector families
...
All figures are single runs at seed 42. We therefore do not claim
statistical significance for any comparison, and small differences in
Table~\ref{tab:indomain}---notably the 0.74-point logistic-regression gain in
...
paper and on structural argument, not on a re-execution of it. Isolating the
per-mechanism contribution of M1 and M2---by re-running our own benchmark under
a random rather than speaker-disjoint split, with and without pre-split
oversampling---would quantify each mechanism directly and is the natural next
step.
...
\texttt{https://github.com/RohamIzadidoost/Kaggle\_deepFakeAudio},
